...
class AppUi extends React.Component {
  ...
  render () {
    ...
    const tabEL1 = <Badge count={alarms.groups[0].count}><span style={{ padding: 12 }}>System</span></Badge>
    const alarmsEL1 = alarms.groups[0].active.map((a, i) => {
      return <Alarm a={a} key={i} />
    })
    // const tabEL2 = <Badge count={alarms.groups[1].count}><span style={{ padding: 12 }}>System (KKP)</span></Badge>
    ...
    return (
      ...
      >
        <Tabs type='card'>
          <Tabs.TabPane tab={tabEL1} key='1'>{alarmsEL1.length > 0 ? alarmsEL1 : <Ready label='System' />}</Tabs.TabPane>
          {/* <Tabs.TabPane tab={tabEL2} key='2'>{alarmsEL2.length > 0 ? alarmsEL2 : <Ready label='System (KKP)' />}</Tabs.TabPane> */}
        </Tabs>
        <style jsx global>{`
          .alarm-alert {
            margin-bottom: 8px;
