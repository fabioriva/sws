...
class AppUi extends React.Component {
  ...
  render () {
    ...
    const tabEL3 = <Badge count={alarms.groups[2].count}><span style={{ padding: 12 }}>System 3</span></Badge>
    const alarmsEL3 = alarms.groups[1].active.map((a, i) => {
      return <Alarm a={a} key={i} />
    })
    const tabEL4 = <Badge count={alarms.groups[3].count}><span style={{ padding: 12 }}>System 4</span></Badge>
    const alarmsEL4 = alarms.groups[1].active.map((a, i) => {
      return <Alarm a={a} key={i} />
    })
    return (
      <Layout
      ...
      >
        <Tabs type='card'>
          <Tabs.TabPane tab={tabEL1} key='1'>{alarmsEL1.length > 0 ? alarmsEL1 : <Ready label='EL1' />}</Tabs.TabPane>
          <Tabs.TabPane tab={tabEL2} key='2'>{alarmsEL2.length > 0 ? alarmsEL2 : <Ready label='EL2' />}</Tabs.TabPane>
          <Tabs.TabPane tab={tabEL3} key='3'>{alarmsEL3.length > 0 ? alarmsEL3 : <Ready label='EL3' />}</Tabs.TabPane>
          <Tabs.TabPane tab={tabEL4} key='4'>{alarmsEL4.length > 0 ? alarmsEL4 : <Ready label='EL4' />}</Tabs.TabPane>
        </Tabs>
        <style jsx global>{`
          .alarm-alert {
            margin-bottom: 8px;
